...
###############################################################################
    ...
    def save_frames(self, data: np.ndarray, data_name: str,
                    del_existing: bool = True) -> None:
    ...
    def filter_data(self, data: np.ndarray) -> np.ndarray:
        ...
        if len(data.shape) != 4 or data.shape[-1] != 1:
            raise ValueError(f"""Passed array ({data.shape}) is not of the
    ...
    def zdisc_info_to_pandas(self, zdiscs_info_all):
        ...
        for i, zdiscs_info_frame in enumerate(zdiscs_info_all):
            ...
            zdiscs_info_frame_extended = np.hstack((frame_id,
                                                    zdisc_id_in_frame,
                                                    zdiscs_info_frame,
                                                    fake_mass))
            data_frames.append(pd.DataFrame(zdiscs_info_frame_extended,
                               columns=['frame', 'zdisc_id', 'x', 'y', 'p1_x',
                                        'p1_y', 'p2_x', 'p2_y', 'mass']))
...
#                             Z-Disc Segmentation                             #
###############################################################################
    def preprocessing(self, input_path: str,
                      save_data: bool = False) -> List[np.ndarray]:
    ...
    def zdisc_detection(self, filtered_frames, save_data=False):
        ...
        for i, frame in enumerate(filtered_frames):
            ...
            valid_contours.append(np.delete(contours,
                                            np.where(contours_size < 8)[0]))
        if save_data:
    ...
    def zdisc_segmentation(self, input_path, save_data=False):
        ...
        if save_data:
            self.save_frames(data=zdiscs_processed_all,
                             data_name='zdiscs-info')
...
###############################################################################
    ...
    def merge_partially_tracked_zdiscs(self, tracked_zdiscs,
                                       partial_tracking_threshold):
        num_frames, partially_tracked_zdiscs, fully_tracked_zdiscs = self.find_fully_tracked_zdiscs(tracked_zdiscs)
        partially_tracked_clusters = partially_tracked_zdiscs[['x', 'y', 'particle']].groupby(by=['particle']).mean()
        ...
        for i, optics_cluster in enumerate(optics_clusters):
            if optics_cluster >= 0:
                ...
                particles_in_cluster = partially_tracked_clusters.iloc[index].index.to_numpy()
                merged_zdiscs = partially_tracked_zdiscs.loc[partially_tracked_zdiscs['particle'].isin(particles_in_cluster)].groupby('frame').mean()
    ...
    def zdisc_tracking(self, partial_tracking_threshold, input_path=None, zdiscs_info=None, tp_depth=4):
        ...
        tracked_zdiscs = tp.filter_stubs(t, int(len(zdiscs_info)*.10)).reset_index(drop=True)
        
        return self.merge_partially_tracked_zdiscs(tracked_zdiscs, partial_tracking_threshold)
        
    #################################################################################
    #							    Sarcomere Detection							    #
    #################################################################################
    # if input_type=='image' then zdisc_clusters are the zdiscs in the only available frame.
    # if input_type=='video' then zdisc_clusters are the average location of a tracked zdisc over all frames.
    def zdisc_clusters_to_graph(self, zdisc_clusters):
        ...
        # create a graph with zdiscs as nodes and connections between a zdisc and its neighbors as edges
        num_nodes = len(zdisc_clusters)
        G = nx.Graph()
        G.add_nodes_from(range(num_nodes))

        # add the position of each zdisc as a node attribute to the graph
        nodes_pos_dict = {i: zdisc_info[0:2] for i, zdisc_info in enumerate(zdisc_clusters)}
        nodes_particle_dict = {i: zdisc_info[-1] for i, zdisc_info in enumerate(zdisc_clusters)}
    ...
    def score_cluster_connections(self, G, c_avg_length=1, c_angle=0.5, c_diff_length=1, l_avg=12.0):
            ...
            for neighbor in G.neighbors(node):
                ...
                for far_neighbor in G.neighbors(neighbor):
                    if far_neighbor in [node, neighbor]:
                        ...
                        v1 = G.nodes[neighbor]['pos'] - G.nodes[node]['pos'] # vector connecting node to neighbor
                        l1 = np.linalg.norm(v1)
                        v2 = G.nodes[far_neighbor]['pos'] - G.nodes[neighbor]['pos'] # vector connecting neighbor to far_neighbor
                        ...
                        score = np.max((score, c_avg_length * avg_length_score + c_diff_length * diff_length_score + c_angle * angle_score))
    ...
    def detect_myofibrils(self, partial_tracking_threshold, input_path=None,
                          fully_tracked_zdiscs=None):
        if input_path is None and fully_tracked_zdiscs is None:
            raise ValueError("""Either input_path to the original video/image
            or trackpy results should be provided.""")
        elif fully_tracked_zdiscs is None:
            fully_tracked_zdiscs = self.zdisc_tracking(partial_tracking_threshold, input_path=input_path)

        fully_tracked_zdiscs_clusters = fully_tracked_zdiscs.groupby('particle').mean().reset_index()[['x', 'y', 'particle']].to_numpy()
        ...
        for myofibril in myofibrils:
            for zdisc_1, zdisc_2 in myofibril.edges:
                sarcs_zdiscs_ids.append([myofibril.nodes[zdisc_1]['particle_id'], myofibril.nodes[zdisc_2]['particle_id']])
        ...
        for i, sarc in enumerate(sarcs_zdiscs_ids):
            for frame in range(frame_num):
                zdisc_1_index = np.where(np.logical_and(fully_tracked_zdiscs.particle==sarc[0], fully_tracked_zdiscs.frame==frame))
                zdisc_1 = fully_tracked_zdiscs.iloc[zdisc_1_index]
                zdisc_2_index = np.where(np.logical_and(fully_tracked_zdiscs.particle==sarc[1], fully_tracked_zdiscs.frame==frame))
                ...
                sarc_info[2, i, frame] = np.linalg.norm(zdisc_1[['x', 'y']].values - zdisc_2[['x', 'y']].values)
                zdisc_1_width = np.linalg.norm(zdisc_1[['p1_x', 'p1_y']].values - zdisc_1[['p2_x', 'p2_y']].values)
                zdisc_2_width = np.linalg.norm(zdisc_2[['p1_x', 'p1_y']].values - zdisc_2[['p2_x', 'p2_y']].values)
                sarc_info[3, i, frame] = np.mean(zdisc_1_width + zdisc_2_width)
                sarc_angle = np.arctan2(zdisc_2.y.values - zdisc_1.y.values,
